...
const create = (options = {}) => {
  ...
  const finalTransformation = constants.getTransformation(
    transformation,
  ...
  const enhanceCloudinaryResponse = (req, res, next) => {
    ...
    try {
      ...
      if (req.file) {
        ...
        logger.debug(
          `[UPLOAD] Uploaded file to Cloudinary: ${req.file.originalname} -> ${req.file.path}`
        );
      }
      ...
      if (req.files) {
        ...
        if (Array.isArray(req.files)) {
          ...
          logger.debug(
            `[UPLOAD] Uploaded ${req.files.length} files to Cloudinary folder: ${folder}`
          );
        }
        // Fields of files
        else {
          ...
          logger.debug(
            `[UPLOAD] Uploaded ${totalFiles} files across ${
              Object.keys(req.files).length
            } fields to Cloudinary folder: ${folder}`
          );
        }
      }
      ...
      logger.error(
        `[UPLOAD] Error processing Cloudinary response: ${error.message}`
      );
      next(error);
    }
  };
