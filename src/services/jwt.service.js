...
import { BadTokenError, InternalServerError } from '../utils/errors/types/Api.error';
...
const JwtService = {
  ...
  jwtVerify: async (token) => {
    ...
    try {
      if (redisService.isConnected()) {
        ...
        const blacklisted = await redisService.get(`jwt:blacklist:${tokenFingerprint}`);
        
        if (blacklisted) {
          logger.warn(`[JWT] Token with fingerprint ${tokenFingerprint} has been revoked`);
          throw new BadTokenError("[JWT] Token has been revoked");
        }
      } else {
        logger.error("[JWT] Redis not connected and blacklist check is required");
        throw new InternalServerError("[JWT] Unable to verify token against blacklist");
      }
    } catch (error) {
      // If this is already a BadTokenError or InternalServerError, rethrow it
      if (error instanceof BadTokenError || error instanceof InternalServerError) {
        throw error;
      }
  ...
  jwtBlacklistToken: async (token) => {
    try {
      ...
      const defaultTtl = parseInt(process.env.JWT_BLACKLIST_DEFAULT_TTL || '86400');
      const fallbackTtl = parseInt(process.env.JWT_BLACKLIST_FALLBACK_TTL || '3600');
      ...
      if (redisService.isConnected()) {
        ...
        await redisService.set(`jwt:blacklist:${tokenFingerprint}`, true, ttl > 0 ? ttl : fallbackTtl);
        logger.info(`[JWT] Added token with fingerprint ${tokenFingerprint} to blacklist with TTL ${ttl > 0 ? ttl : fallbackTtl}`);
      } else {
        logger.warn(`[JWT] Redis not connected, could not blacklist token ${tokenFingerprint}`);
      }
    } catch (error) {
