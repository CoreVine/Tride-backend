...
const schoolSchema = Yup.object().shape({
  ...
  lat: Yup.number()
    ...
    .test(
      "is-decimal",
      "Latitude must be a decimal number with up to 6 decimal places",
      (value) => {
  ...
  lng: Yup.number()
    ...
    .test(
      "is-decimal",
      "Longitude must be a decimal number with up to 6 decimal places",
      (value) => {
        if (value === undefined || value === null) return false;
        const decimalRegex = /^-?\d+(\.\d{1,6})?$/;
        return decimalRegex.test(value.toString());
...
schoolRoutes
  ...
  .put(validate(schoolUpdateSchema), schoolController.updateschool)
  .delete(schoolController.deleteSchollByID);
