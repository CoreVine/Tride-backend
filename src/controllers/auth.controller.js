...
const authController = {
  register: async (req, res, next) => {
    try {
      ...
      if (accountExists) {
        logger.warn('Account registration failed - email already exists', { email });
  ...
  login: async (req, res, next) => {
    try {
      const { email, password, account_type } = req.body;
      logger.info(`Login attempt for: ${email} as ${account_type || 'unspecified type'}`);
      ...
      if (requestedType !== account.account_type) {
        logger.warn(`Login failed - account type mismatch: ${email} requested ${requestedType} but is ${account.account_type}`);
  ...
  me: async (req, res, next) => {
    try {
      ...
      if (accountType === 'parent') {
        const parent = await ParentRepository.findByAccountIdWithChildren(account.id);
        if (parent) {
          profileData.parent = parent;
          profileComplete = true;
